<template>
  <v-dialog persistent  v-model="editDialogOpen" max-width="500">
    <v-card class="text-xs-center">
      <v-card-title class="headline justify-center" >เพิ่มปลัดอำเภอ</v-card-title>
      <v-card-text>
        <v-layout row pt-3>
          <v-flex>
            <v-text-field
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions('deputyDistrictStore', ['updateDeputy']),
      onDialogClose () {
      ...
      initialData() {
        ...
        this.selectedAA= this.selectedPerson.catmUkey.substring(2,4)
